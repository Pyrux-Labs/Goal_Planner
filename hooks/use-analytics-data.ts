...
export function useAnalyticsData(
    ...
    const [result, setResult] = useState<
        Omit<AnalyticsResult, "loading" | "error">
    >({
        tasksCompleted: 0,
        tasksPending: 0,
        habitsCompleted: 0,
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const [{ data: tasksData }, { data: habitsData }] =
                await Promise.all([
                    supabase
                        .from("tasks")
                        .select("id, goal_id")
            ...
            const tasksCompleted = tLogs.filter(
                (l: { completed: boolean; date: string }) =>
                    l.completed && l.date <= today,
            ).length;
            const tasksPending = tLogs.length - tasksCompleted;
            const habitsCompleted = hLogs.filter(
                (l: { completed: boolean; date: string }) =>
                    l.completed && l.date <= today,
            ).length;
            ...
            tLogs.forEach((l: { completed: boolean; date: string }) => {
                if (l.completed && l.date <= today)
                    taskActivityMap[l.date] =
                        (taskActivityMap[l.date] || 0) + 1;
            });
            hLogs.forEach((l: { completed: boolean; date: string }) => {
                if (l.completed && l.date <= today)
                    habitActivityMap[l.date] =
                        (habitActivityMap[l.date] || 0) + 1;
            });
            ...
            (tasksData || []).forEach((t: { id: number; goal_id: number }) =>
                taskGoalMap.set(t.id, t.goal_id),
            );
            const habitGoalMap = new Map<number, number>();
            (habitsData || []).forEach((h: { id: number; goal_id: number }) =>
                habitGoalMap.set(h.id, h.goal_id),
            );
            ...
            >();
            goalsList.forEach((g: { id: number }) =>
                goalStats.set(g.id, { completed: 0, total: 0 }),
            );

            (allTaskLogs || []).forEach(
                (l: { task_id: number; completed: boolean }) => {
            ...
            (allHabitLogs || []).forEach(
                (l: { habit_id: number; completed: boolean }) => {
            ...
            const [{ data: streakTasks }, { data: streakHabits }] =
                await Promise.all([
                    taskIds.length > 0
                        ? supabase
                              .from("task_logs")
            ...
            (streakTasks || []).forEach((l: { date: string }) =>
                completedDates.add(l.date),
            );
            (streakHabits || []).forEach((l: { date: string }) =>
                completedDates.add(l.date),
            );
