...
import { goalHasTasksOrHabits } from "@/lib/services/goal-service";
...
function OnboardingContent() {
	...
	const handleNext = async () => {
		...
		if (currentStep === 2) {
			...
			try {
				const hasItems = await goalHasTasksOrHabits(currentGoalId);
						...
						<main className="pt-4 pb-28 overflow-y-auto mx-4 md:mx-12 lg:mx-28">
							...
							{onboardingError && (
								<div ref={errorRef} className="mt-4 mb-6">
									<ErrorMessage message={onboardingError} variant="block" />
								</div>
							)}
						</main>
						<NavigationButtons
