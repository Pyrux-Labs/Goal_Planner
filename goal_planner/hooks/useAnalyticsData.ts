/**
 * Hook centralizado de analytics.
 * Reemplaza useStatsData y provee datos para /stats y el sidebar.
 * Ambos consumidores usan este hook — no duplicar lógica de fetch.
 */
...
/** Actividad por día — tareas y hábitos por separado para el gráfico apilado */
...
function getPeriodRange(
    ...
    // monthly — usa refYear/refMonth como referencia
...
/** Genera el array de días para el gráfico según el período */
...
export function useAnalyticsData(
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            // ── Period logs (para stats del período) ───────────────────────
            // Traemos TODOS los logs del período (incluyendo días futuros pre-generados por el RPC)
            // para que el denominador del donut sea el total programado en el período.
            // "completed" se filtra en cliente: completed=true AND date <= today.
            // "pending" = total del período − completed (incluye días futuros aún no vencidos).
            // Para "all" no hay rango fijo, así que acotamos a hoy.
            ...
            if (range) {
                ...
                // "all" — todos los logs hasta hoy (no hay rango futuro)
                taskLogsQ = taskLogsQ.lte("date", today);
                habitLogsQ = habitLogsQ.lte("date", today);
            }

            // ── All-time logs (para goal progress) ─────────────────────────
            const [
                { data: periodTaskLogs },
            ...
            // ── Stats del período ───────────────────────────────────────────
            // tLogs/hLogs incluyen días futuros pre-generados (completed=false).
            // "completed" = marcado como hecho Y ya pasó (date <= today).
            // "pending"   = total del período − completed (incluye futuros no vencidos).
            ...
            // ── Daily activity (para gráfico apilado) ──────────────────────
            // Solo días <= today (tLogs puede incluir futuros, los ignoramos aquí)
            ...
            // ── Streak (últimos 90 días, por user_id) ──────────────────────
