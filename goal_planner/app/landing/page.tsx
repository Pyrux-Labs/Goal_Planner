...
function LandingContent() {
	...
	const [isCheckingAuth, setIsCheckingAuth] = useState(true); // NUEVO
	...
	// Check authentication status - ACTUALIZADO
	useEffect(() => {
		const checkAuth = async () => {
			...
			if (session?.user) {
				...
				// Si hay usuario, redirigir al calendar
		...
		// Escuchar cambios de autenticación - NUEVO
		const supabase = createClient();
		const {
			data: { subscription },
			...
			if (event === "SIGNED_IN" && session?.user) {
				...
				setUser(session.user);
				// Redirigir al calendar después del login
				router.push("/calendar");
			}
	...
	// Mostrar loading mientras verifica autenticación - NUEVO
	if (isCheckingAuth) {
		return (
			<div className="bg-landing-bg min-h-screen w-full flex items-center justify-center">
				<div className="text-white-pearl text-xl">Loading...</div>
			</div>
		);
	}
