...
export default function OnboardingPage() {
	...
	const handleNext = async () => {
		...
		if (currentStep === 2) {
			...
			// If goal already created, validate tasks/habits before proceeding
			else {
				try {
					const supabase = createClient();

					// Check if goal has at least one task or habit
					...
					console.error("Error validating tasks/habits:", error);
					setOnboardingError("Failed to validate goal. Please try again.");
					return;
				}
			}
		}
									...
									return (
										<GoalCard
											...
											onTaskEdit={(taskIndex) =>
												console.log(`Edit task ${taskIndex} from ${goal.name}`)
											}
