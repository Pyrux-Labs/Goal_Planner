...
import { deleteOwnAccount, requireUserId } from "@/lib/services/authService";
import {
	bulkDeleteUserTasks,
...
export default function SettingsPage() {
	...
	const handleBulkDelete = async () => {
		...
		try {
			...
			if (bulkDeleteTarget === "tasks") {
				await bulkDeleteUserTasks(userId);
			} else if (bulkDeleteTarget === "habits") {
				await bulkDeleteUserHabits(userId);
			} else if (bulkDeleteTarget === "goals") {
				await bulkDeleteUserGoals(userId);
			}
