...
import { updateTaskLog, updateHabitLog } from "@/lib/services/log-service";
		...
		<div className="space-y-4 px-4">
			...
			<div>
				<label className="block text-white-pearl mb-2 text-sm">
					Start Time <span className="text-input-text text-xs">(Optional)</span>
				</label>
				<input
					type="time"
					value={editStartTime}
			...
			<div>
				<label className="block text-white-pearl mb-2 text-sm">
					End Time <span className="text-input-text text-xs">(Optional)</span>
				</label>
				<input
					type="time"
					value={editEndTime}
...
export function EditHabitLog({
	...
	const handleSubmit = async () => {
		...
		try {
			await updateHabitLog(logId, editDate);
			onClose();
		} catch {
	...
	return (
		<div className="space-y-4 px-4">
			<p className="text-white-pearl/70 text-sm">
				Editing log for:{" "}
				<span className="text-white-pearl font-medium">{habitName}</span>
			</p>
