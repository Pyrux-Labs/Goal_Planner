...
import { deleteTaskLog } from "@/lib/services/task-service";
import { deleteHabitLog } from "@/lib/services/habit-service";
import { useToggleEvent } from "@/hooks/use-toggle-event";
import { useToast } from "@/components/ui/toast-context";
import { sortEventsByTime } from "@/lib/date-utils";
import { formatTimeRange } from "@/lib/format-utils";
...
const CalendarInfo = ({
	...
	const deleteUpdatingIdsRef = useRef<Set<number>>(new Set());
	...
	const handleDeleteLog = useCallback(
		async (event: CalendarEvent, isHabit: boolean) => {
			if (deleteUpdatingIdsRef.current.has(event.id)) return;
			deleteUpdatingIdsRef.current.add(event.id);
			setDeleteUpdatingIds(new Set(deleteUpdatingIdsRef.current));
			...
			deleteUpdatingIdsRef.current.delete(event.id);
			setDeleteUpdatingIds(new Set(deleteUpdatingIdsRef.current));
		},
		[onRefresh, showToast],
	);
	...
	return (
		<div className="h-full overflow-y-auto p-4 space-y-4 scrollbar-custom">
			...
			{/* Empty State */}
			{totalEvents === 0 && (
				<div className="text-sm text-input-text text-center py-4">
